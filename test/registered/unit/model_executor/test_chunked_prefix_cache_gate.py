...
class TestChunkedPrefixCacheGate(CustomTestCase):
    ...
    def test_republish_discards_the_gate_so_it_must_run_after_publish(self):
        # Publishing rebuilds the bags from the pristine instance and clears
        # the overrides log, so a publish AFTER the gate silently reverts it.
        # This pins the ordering contract in ModelRunner.__init__: the gate
        # runs after the target-worker publish, never before.
        self._seed(attention_backend="triton")
        sa = get_server_args()
        maybe_disable_chunked_prefix_cache(use_mla_backend=True, is_draft_worker=False)
        self.assertTrue(get_schedule().disable_chunked_prefix_cache)
    ...
    def test_draft_copy_overrides_carry_the_gate(self):
        # The draft ServerArgs copy comes from the pristine instance, which
        # never sees the bag-only gate — the copy's pre-publish overrides must
        # carry the resolved value or the draft's private bags desync.

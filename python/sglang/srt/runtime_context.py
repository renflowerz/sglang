    ...
    def _check_role_namespace(self, name: str) -> None:
        ...
        if _ROLE_NS_MODE == "record":
            ...
            if role not in ROLE_NAMESPACE_SETS:
                # Fail closed: an unknown (or misspelled) role must not
                # silently inherit the full tree.
                raise ValueError(
...
def _role_ns_mode_from_env() -> str:
    # Registered on Envs per project convention; resolved once at import so
    # the config_bag gate stays a dynamo-prunable constant. Validation stays
    # fail-loud here (EnvField's warn-and-default parse would silently turn a
    # typo into "off").
...
def _append_role_ns_out(role: str | None, name: str) -> None:
    ...
    try:
        ...
        # The entry stays in the in-memory set (the exit summary still covers
        # it); surface the persistence gap so an operator does not seed an
        # enforcement set from a silently incomplete file.
        print(
...
def publish(server_args, *, role: str, hf_config: Any = None) -> RuntimeContext:
    ...
    **last-publish-wins** (bags re-projected, provenance reset, role
    ...
    if _ROLE_NS_MODE == "enforce" and role not in ROLE_NAMESPACE_SETS:
        # Fail closed at publish time, not at the first stray read: an unknown
        # (or misspelled) role must not silently inherit the full tree.
        raise ValueError(
    ...
    if _ROLE_NS_MODE == "record":
        # A zero-read role must still be distinguishable from a process where
        # recording never ran: persist a '-' marker now (signal teardown skips
        # atexit) and register the exit summary.
